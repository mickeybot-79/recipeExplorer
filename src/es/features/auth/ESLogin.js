...
import { useLoginMutation, useVerifyUsernameMutation } from '../../../features/auth/authApiSlice'
                ...
                setVerifyOption(() => {
                    ...
                    if (result?.data?.match === 'true') {
                        ...
                        setRecipeOptions(() => {
                            ...
                            optionElements.push(<option key={0} value="" readOnly>No he publicado recetas</option>)
        ...
        <section id='login-container'>
            ...
            <main id="login-form-container">
                <form id="login-form" onSubmit={handleSubmit}>
                    ...
                    <label htmlFor="persist" className="form-persist-es">
                        <input
                            type="checkbox"
                    ...
                    <button id="form-submit-button">Iniciar sesión</button>
                    <div id='create-account-container'>
                        <Link to="/es/signup">¿Aún no tienes un usuario? Crear nueva cuenta</Link>
                    </div>
                </form>
                <div id='verify-account-container' style={{ display: displayVerify }}>
                    <div id='verify-username-container' style={{ display: verifyOption === 'prompt' ? 'grid' : 'none', animation: userPromptAnimation }}>
                        <p>Por favor, escribe tu nombre de usuario: </p>
                        <input
                            className="form-input"
                            type="text"
                            id="verify-username"
                    ...
                    <div id='new-pwd-container' style={{ display: verifyOption === 'new-pwd' ? 'grid' : 'none', animation: userPromptAnimation }}>
                        <p>Por favor, crea una nueva contraseña: </p>
                        <input
                            className="form-input"
                            type="password"
                            id="new-pwd"
                        ...
                        />
                        <p style={{display: pwdMismatch, fontSize: '15px', marginTop: '-15px', marginBottom: '-10px', color: 'red'}}>Las contraseñas no coinciden</p>
                        <div id='verify-buttons-container'>
                            <button type='button' id='cancel-verify' onClick={() => {
                                setUserPromptAnimation(() => {
                                    return 'new-collection-out 0.2s linear 1'
                    ...
                    <div id='unverified-container' style={{ display: verifyOption === 'unverified' ? 'grid' : 'none', animation: userPromptAnimation }}>
                        <p>No pudimos confirmar tu identidad.</p>
                        <p>¿Puedes seleccionar una receta publicada por ti?</p>
                        <select defaultValue='select' ref={selectRef}>
                            <option value='select' hidden readOnly>Selecciona una opción</option>
                            {recipeOptions}
                        </select>
                    ...
                    <div id='failed' style={{ display: verifyOption === 'failed' ? 'grid' : 'none', animation: userPromptAnimation }}>
                        <p>Par desgracia, no podemos confirmar tu identidad ahora.</p>
                        <Link to="/signup">Por favor, crea una cuenta aquí ➜</Link>
                    </div>
                </div>
